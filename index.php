...
if(isset($_GET["ajax"])){
    ...
    if(
        !isset($_SESSION["recover1_token"]) && 
        !isset($_SESSION["recover2_token"]) && 
        !isset($_SESSION["auth_token"]) && 
        !isset($_SESSION["register_token"])  
    ){
        $rand = rand(100000, 1000000);
        $_SESSION["recover1_token"] = $rand;
        $_SESSION["recover2_token"] = $rand;
        $_SESSION["auth_token"] = $rand;
        $_SESSION["register_token"] = $rand;
        $_SESSION["token_time"] = $time;
    }

    if(isset($_SESSION["token_time"]) && ($time-$_SESSION["token_time"]) > 180){
        $rand = rand(100000, 1000000);
        $_SESSION["recover1_token"] = $rand;
        $_SESSION["recover2_token"] = $rand;
        $_SESSION["auth_token"] = $rand;
