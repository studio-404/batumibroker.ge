...
define('WEBSITE_BASE', 'http://batumibroker.ge/');
define('CMS', '_manager');
...
// Connect to DB
$db_link = mysql_pconnect(c('database.hostname'), c('database.username'), c('database.password')) OR exit('Could not connect to database.');
...
{
	$language_sql = db_fetch("SELECT * FROM ".c('table.languages')." WHERE `shortname` = '" . $language . "';");
    return isset($language_sql["name"]) ? $language_sql["name"] : NULL;
}
