...
//		$menu=db_fetch("select * from ".c("table.menus")."` where `id`='".$this->storage->section['menuid']."' and `language`='".l()."'");
        ...
        $author = db_retrieve('title', c("table.pages"), 'attached', $mtitle, ' and language = "'.l().'" LIMIT 1');
...
//        $tpl['id'] = $this->storage->section['id'];
        ...
        $sql = "SELECT * FROM `".c("table.pages")."` WHERE `deleted` = '0' AND language = '" . l() . "' {$all_news}{$filter}AND visibility = 'true' ORDER BY pagedate DESC;";
        $res = db_fetch_all($sql);
//        if (empty($res))
//            return ($this->storage->content = NULL);
        if ($rss)
    ...
    function photo()
    {
        $sql_c = "SELECT * FROM `".c("table.pages")."` WHERE `deleted` = '0' AND `masterid` = '{$this->storage->section['idx']}' order by pagedate DESC;";
        $res_c = db_fetch_all($sql_c);

		if(!empty($res_c)) {
			$photos["title"] = $this->storage->section['title'];
			$photos["imagen"] = $this->storage->section['imagen'];
...
//        $menu_id = db_retrieve('id', c("table.menus"), 'deleted', '0', "AND `type` = 'audiogallery' AND `name` = '{$this->storage->section['attached']}'");
    ...
    function poll()
    {
     ...
     function catalog()
    {
        if(isset($_GET["product"])) { // go to product page
        ...
        $sql = "SELECT 
        (SELECT count(*) FROM `".c("table.catalogs")."` WHERE `catalogid` = {$menu_id} AND `deleted`=0 AND `visibility`='true' AND `language` = '" . l() . "') as count, 
        ...
        (SELECT `site_users`.`firstname` FROM `site_users` WHERE `site_users`.`active`=1 AND `site_users`.`banned`=0 AND `site_users`.`id`=`catalogs`.`administrator`) as admin_firstname, 
        (SELECT `site_users`.`phone` FROM `site_users` WHERE `site_users`.`active`=1 AND `site_users`.`banned`=0 AND `site_users`.`id`=`catalogs`.`administrator`) as admin_phone1, 
        (SELECT `site_users`.`mobile` FROM `site_users` WHERE `site_users`.`active`=1 AND `site_users`.`banned`=0 AND `site_users`.`id`=`catalogs`.`administrator`) as admin_phone2, 
        (SELECT `site_users`.`skype` FROM `site_users` WHERE `site_users`.`active`=1 AND `site_users`.`banned`=0 AND `site_users`.`id`=`catalogs`.`administrator`) as admin_skype, 
        ...
        (SELECT count(`id`) FROM `".c("table.catalogs")."` WHERE `catalogid` IN(7,8,9,10,11) AND `deleted`=0 AND `visibility`='true' AND `language` = '" . l() . "' and `administrator`=admin_id) as admin_ads, 
        `catalogs`.* 
        FROM 
        `".c("table.catalogs")."` 
        WHERE 
        `catalogid` = {$menu_id} AND 
        `deleted`=0 AND 
        `visibility`='true' AND 
        `language` = '" . l() . "' 
		...
		$catalog["idx"] = $this->storage->section['idx'];
        $catalog["title"] = $this->storage->section['title'];
        $catalog["slug"] = $this->storage->section['slug']; 
