    ...
    function catalog()
	{
        ...
        switch ($this->subaction):
            case 'show':
                ...
                $sql = db_fetch_all("SELECT * FROM `".c("table.catalogs")."` WHERE `deleted` = 0 and `language` = '".l()."' and `catalogid` = ". get("menu", 0) ." ORDER BY `position` DESC;");
				$data["title"] = $catalog["title"];
                $data["catalogs"] = $sql;
                $this->content = template('actions/' . $this->action . '_list', $data);
                break;
            case 'add':
    //         	$mask = __DIR__.'_cache_html/*.*';
				// array_map('unlink', glob($mask));
                
                if (isset($_POST[$this->action . '_form_perform']) && !empty($_POST['title'])):
					$id=db_fetch('select max(id) as maxid from `'.c("table.catalogs").'`');
					$vis = ((isset($_POST["visibility"])) && ($_POST["visibility"] == 'on')) ? 'true' : 'false';
					$supervip = ((isset($_POST["supervip"])) && ($_POST["supervip"] == 'on')) ? '1' : '0';
					$vip = ((isset($_POST["vip"])) && ($_POST["vip"] == 'on')) ? '1' : '0';
					$slider = ((isset($_POST["slider"])) && ($_POST["slider"] == 'on')) ? '1' : '0';
					$theStatus = (isset($_POST["status"])) ? $_POST["status"] : ' ';
					$possibleexchange = ((isset($_POST["possibleexchange"])) && ($_POST["possibleexchange"] == 'on')) ? '1' : '0';
					$short_description = (!empty($_POST["short_description"])) ? $_POST["short_description"] : ' ';
					$cadastralcode = (!empty($_POST["cadastralcode"])) ? $_POST["cadastralcode"] : ' ';
					$sale = (!empty($_POST["sale"])) ? (int)$_POST["sale"] : ' ';
					$city = (!empty($_POST["city"])) ? (int)$_POST["city"] : ' ';
					$rooms = (!empty($_POST["rooms"])) ? $_POST["rooms"] : ' ';
					$price = (!empty($_POST["price"])) ? $_POST["price"] : ' ';
					$address = (!empty($_POST["address"])) ? $_POST["address"] : ' ';
					$currency = (!empty($_POST["currency"])) ? $_POST["currency"] : 'GEL';
					$floor = (!empty($_POST["floor"])) ? $_POST["floor"] : ' ';
					$floor_all = (!empty($_POST["floor_all"])) ? $_POST["floor_all"] : ' ';
					$condition = (!empty($_POST["condition"])) ? (int)$_POST["condition"] : ' ';
					$project = (!empty($_POST["project"])) ? (int)$_POST["project"] : ' ';
					$square = (!empty($_POST["square"])) ? $_POST["square"] : ' ';
					$bathrooms = (!empty($_POST["bathrooms"])) ? $_POST["bathrooms"] : ' ';
					$ceilheight = (!empty($_POST["ceilheight"])) ? $_POST["ceilheight"] : ' ';
					$aditionalinformation = (isset($_POST["aditionalinformation"]) && is_array($_POST["aditionalinformation"])) ? implode(",", $_POST["aditionalinformation"]) : ' ';
					$map_coordinates = (!empty($_POST["map_coordinates"])) ? $_POST["map_coordinates"] : ' ';
					$contactphone = (!empty($_POST["contactphone"])) ? $_POST["contactphone"] : ' ';
...
//					$_POST["productdate"] .= ' ' . $producttime;
                ...
                $statusTypes = db_fetch_all("SELECT `id`, `title` FROM `".c("table.catalogs")."` WHERE `catalogid`=30 AND `visibility`='true' AND `deleted`=0 AND `language`='".l()."' ORDER BY `position` ASC;");
                ...
                $this->content = template('actions/' . $this->action . '_forms', $add_array);
                break;
            case 'edit':
    //         	$mask = __DIR__.'_cache_html/*.*';
				// array_map('unlink', glob($mask));
                if (isset($_POST[$this->action . '_form_perform']) && !empty($_POST['title'])):
                    ...
                    $data = array(
						...
						'floor' => (!empty($_POST['floor']) ? $_POST['floor'] : ' '),	
						'floor_all' => (!empty($_POST['floor_all']) ? $_POST['floor_all'] : ' '),	
						'condition' => (!empty($_POST['condition']) ? (int)$_POST['condition'] : '0'),	
                ...
                $edit = array(
					...
					'date' => $edit_data['date'],	
					'expire_date' => $edit_data['expire_date'],	
					'cadastralcode' => $edit_data['cadastralcode'],
					'supervip' => $edit_data['supervip'],
					'vip' => $edit_data['vip'],
					'slider' => $edit_data['slider'],
					'possibleexchange' => $edit_data['possibleexchange'],
					'sale' => (int)$edit_data['sale'],
					'status' => (int)$edit_data['status'],
					'city' => (int)$edit_data['city'], 
					'rooms' => $edit_data['rooms'],
					'rooms' => $edit_data['rooms'],
					'price' => $edit_data['price'],		
					'address' => $edit_data['address'],		
					'currency' => $edit_data['currency'],		
					'floor' => $edit_data['floor'],	
					'floor_all' => $edit_data['floor_all'],	
					'condition' => (int)$edit_data['condition'],	
					'project' => (int)$edit_data['project'],	
					'square' => $edit_data['square'],	
					'bathrooms' => $edit_data['bathrooms'],	
					'ceilheight' => $edit_data['ceilheight'],	
					'aditionalinformation' => $edit_data['aditionalinformation'],	
					'map_coordinates' => $edit_data['map_coordinates'],
					'contactphone' => $edit_data['contactphone'],
					'description' => $edit_data['description'],
					'visibility' => $edit_data["visibility"],
					...
					'photo7' => $edit_data['photo7'],
					'photo8' => $edit_data['photo8'],
					'photo9' => $edit_data['photo9'],
					'photo10' => $edit_data['photo10'],
					'idx' => $edit_data['idx'],
					...
					'projects' => $projects,   			
					'aditional_info' => $aditional_info,   			
					'administrator' => $edit_data['administrator']		
                );
                $this->content = template('actions/' . $this->action . '_forms', $edit);
                break;
            case 'visibility':
